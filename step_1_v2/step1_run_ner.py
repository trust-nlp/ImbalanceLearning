# Meta-Resample/meta_aug_run_ner.py
...
class CustomDataCollatorForTokenClassification(DataCollatorForTokenClassification):
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if features and "orig_idx" in features[0]: # Check if features is not empty
            logger.info(f"[DEBUG_COLLATOR] features[0].keys() before pop: {features[0].keys()}")
            original_indices = [feature.pop("orig_idx") for feature in features]
            logger.info(f"[DEBUG_COLLATOR] Successfully popped 'orig_idx'. features[0].keys() after pop: {features[0].keys() if features else 'features_empty'}")
        elif features:
        ...
        if features:
            ...
            if keys_to_remove_before_super_call:
                logger.info(f"[DEBUG_COLLATOR] Removing keys {keys_to_remove_before_super_call} before calling super collator.")
        ...
        if original_indices is not None:
            ...
            logger.info(f"[DEBUG_COLLATOR] Added 'orig_idx' to batch. Batch keys: {batch.keys()}")
        else:
