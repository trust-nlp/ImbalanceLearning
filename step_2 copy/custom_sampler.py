...
class HardnessAwareSampler(Sampler):
    ...
    def __init__(self, 
        ...
        Args:
            ...
            ema_losses: A tensor of EMA losses for each sample in the dataset.
                        Expected to be on the CPU initially, and updated by the Trainer.
            alpha: Exponent for scaling losses. Higher alpha focuses more on hard examples.
            epsilon: Small constant to add to losses before exponentiation to avoid log(0) or division by zero.
            p_max: Optional. Maximum sampling probability for any single sample.
            num_samples: The number of samples to draw per epoch. If None, defaults to dataset_len.
            neighbor_boost: Optional. Tensor of neighbor boosts for each sample. (Expected to be on Trainer's device, e.g. GPU)
            lambda_boost: Float. Scaling factor for neighbor boosts.
        """
    ...
    def _calculate_probabilities(self) -> torch.Tensor:
        ...
        if self.neighbor_boost is not None:
            # neighbor_boost is expected to be on the same device as trainer.args.device (e.g. GPU)
            # We need to bring it to CPU if weights are on CPU, or ensure weights move to its device if applicable.
            # Here, weights are already on CPU due to current_ema_losses.cpu().float()
    ...
    def set_neighbor_boost(self, boost_tensor):
        ...
        # This method might not be needed anymore if direct tensor reference works as expected
        # self.neighbor_boost = boost_tensor.cpu() # Old behavior, ensuring CPU tensor
        # For direct reference, this method should probably update the content if it were to be used
        # For now, let's log if it's called, as it shouldn't be with the new design.
        ...
        # If we absolutely need to keep it functional for some reason:
        # self.neighbor_boost.copy_(boost_tensor) # This would copy content, assuming boost_tensor is on correct device
