...
import sys, os
# 确保无论当前工作目录是什么，都能 import 到 本地的 meta 包
...
class HardnessArgs:
    ...
    hardness_alpha: float = field(default=1.0)
    knn_k: int = field(default=5)
    knn_lambda: float = field(default=0.5)
    knn_build_freq: int = field(default=1)
    vnet_lr: float = field(default=1e-4)
    meta_update_lr: float = field(default=2e-4)
    meta_update_scale_factor: float = field(default=2.0)

    # === NEW: directory holding *.npy sentence embeddings ===
    embedding_dir: Optional[str] = field(
        default=None,
        metadata={"help": "Dir that stores pre-computed sentence-level embeddings (train_embeddings.npy etc.)"},
    )
...
def main():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, TrainingArguments, HardnessArgs)
    )
    ...
    # ----- 强制启用 Hardness-Aware Sampling，因为这是此脚本的核心 -----
    if not hardness_args.hardness_aware_sampling:
        raise ValueError(
            "This script is designed for hardness-aware sampling. "
            "Please run with '--hardness-aware-sampling true'."
        )
    logger.info("Hardness-aware sampling is enabled via command line.")
    ...
    # 总是导入 HardnessAwareTrainer，不再依赖条件判断
    from meta.custom_trainer import HardnessAwareTrainer, KnnEpochEndCallback

    # Setup logging
    logging.basicConfig(
        level=logging.INFO,  # ★ 新增：让 INFO 级别可见
        format="%(asctime)s - %(levelname)s - %(name)s - %(message)s",
        datefmt="%m/%d/%Y %H:%M:%S",
        handlers=[logging.StreamHandler(sys.stdout)],
    )
    logging.getLogger("meta").setLevel(logging.INFO)  # ★ 可选：只打开 meta 包日志
    ...
    config = AutoConfig.from_pretrained(
        ...
        finetuning_task="relation-extraction",
        cache_dir=model_args.cache_dir,
    ...
    # For relation extraction, update the config with label infos
        ...
        for tokens, entities, relations in zip(
            ...
            for r in relations:
                ...
                try:
                    ...
                    logger.warning(f"[SKIP-POS] In sentence, failed to build instance: {e}")
                    continue
            ...
            if data_args.negative_ratio > 0:
                ...
                # **【关键修复】** 如果没有正样本，我们仍然可以从这个句子中采样负样本，而不是直接跳过。
                # 这确保了不包含任何已知关系的数据也能被模型学到，作为no_relation的来源。
                # 这里我们设定，即使没有正样本，也最多采样2个负样本。
                if num_pos == 0:
                    num_neg_to_sample = 2
                ...
                if num_neg_to_sample > 0:
                    ...
                    for h_id, t_id in neg_samples:
                        try:
                            ...
                            logger.warning(f"[SKIP-NEG] In sentence, failed to build instance: {e}")
                            continue
                        flat_sentences.append(sent)
    ...
    if training_args.do_train:
        ...
        if use_hardness_sampling:
            train_dataset = train_dataset.add_column(
                "orig_idx", list(range(len(train_dataset)))
            )
            META_RATIO = 0.1
            meta_size = max(1, int(len(train_dataset) * META_RATIO))
            meta_dataset = train_dataset.shuffle(seed=42).select(range(meta_size))
        else:
            meta_dataset = None

    # ---------- NEW: sentence‑level embeddings ----------
    if use_hardness_sampling:
        if hardness_args.embedding_dir is None:
            raise ValueError("--embedding_dir 必须指定 (hardness-aware sampling 已启用)")

        emb_path = os.path.join(hardness_args.embedding_dir, "train_embeddings.npy")
        _emb_np = None
        if os.path.isfile(emb_path):
            _emb_np = np.load(emb_path)

        # 若不存在文件或样本数不匹配 → 重新计算并保存
        if _emb_np is None or _emb_np.shape[0] != len(train_dataset):
            logger.warning(
                f"[Embeddings] 需要重建嵌入：现有={_emb_np.shape[0] if _emb_np is not None else 0} "
            ...
            # 直接从 input_ids 反解文本；与训练样本一一对应
            sentences = tokenizer.batch_decode(
                train_dataset["input_ids"], skip_special_tokens=True
            )

            _emb_np = sbert.encode(
                sentences,
                batch_size=64,
            ...
            os.makedirs(hardness_args.embedding_dir, exist_ok=True)
            np.save(emb_path, _emb_np)
            logger.info(f"[Embeddings] 已保存重建文件 {emb_path}, shape={_emb_np.shape}")
        else:
            logger.info(f"[Embeddings] 成功加载 {emb_path}, shape={_emb_np.shape}")

        train_sentence_embeddings = torch.from_numpy(_emb_np).float()
    ...
    # SequenceTrainer 功能已合并到 HardnessAwareTrainer 中
    ...
    # Initialize our Trainer - 无条件使用 HardnessAwareTrainer
    logger.info("Initializing HardnessAwareTrainer with meta-learning parameters...")
    logger.info(f"[DEBUG] use_hardness_sampling = {use_hardness_sampling}")
    logger.info(f"[DEBUG] hardness_args.hardness_aware_sampling = {hardness_args.hardness_aware_sampling}")
    logger.info(f"[DEBUG] training_args.do_train = {training_args.do_train}")
    trainer = HardnessAwareTrainer(
        ...
        # 传递所有参数，Trainer 内部会根据 hardness_aware_sampling 决定是否使用它们
        loss_name=model_args.loss_name,  # 来自 SequenceTrainer 的功能
        
        # Hardness-specific parameters
        hardness_aware_sampling=use_hardness_sampling,  # 关键开关
        meta_dataset=meta_dataset if use_hardness_sampling else None,
        train_dataset_len=len(train_dataset) if use_hardness_sampling and training_args.do_train else 0,
        ...
        precomputed_embeddings=train_sentence_embeddings if use_hardness_sampling else None,
    )
    
    # 如果启用了 hardness sampling, 才添加 callback
    if use_hardness_sampling:
        knn_callback = KnnEpochEndCallback(trainer_instance=trainer)
        trainer.add_callback(knn_callback)
        logger.info("KnnEpochEndCallback has been added to the trainer.")
