...
from custom_trainer import HardnessAwareTrainerForCls
...
class ExtraTrainingArguments(TrainingArguments):
    hardness_aware_sampling: bool = field(default=False)
    ...
    knn_hard_sample_ratio: float = field(
        default=0.2, metadata={"help": "The ratio of top-weighted samples to be considered 'hard' for KNN neighbor boosting."}
    )
...
# Global list to store real label names, used by compute_metrics.
real_label_names: list[str] = []
...
def main():
    ...
    if data_args.label_column_name is not None and data_args.label_column_name != "label":
        for key in raw_datasets.keys():
            if "label" in raw_datasets[key].column_names:
                raw_datasets[key] = raw_datasets[key].remove_columns("label")
    ...
    if is_regression:
        ...
        # If "label_text" field exists, extract real label names.
        global real_label_names
        if "label_text" in raw_datasets["train"].column_names:
            # Create index -> name mapping, keeping first occurrence for duplicate indices.
            mapping = {}
            for l, t in zip(raw_datasets["train"]["label"], raw_datasets["train"]["label_text"]):
                if l != -1 and l not in mapping:
                    mapping[int(l)] = str(t)
            # Use index as name for missing labels.
            real_label_names = [mapping.get(i, str(i)) for i in range(num_labels)]
        else:
            # Fallback: use the label list directly.
            real_label_names = [str(x) for x in label_list]
    ...
    # Record original indices for HardnessAwareSampler
    def _add_orig(example, idx):
        example["orig_idx"] = idx
        return example
    with training_args.main_process_first(desc="add orig_idx"):
        raw_datasets["train"] = raw_datasets["train"].map(_add_orig, with_indices=True)
    ...
    if training_args.do_predict or data_args.test_file is not None:
        ...
        if "label" in predict_dataset.features:
            predict_dataset = predict_dataset.filter(lambda ex: ex["label"] != -1)
        if data_args.max_predict_samples is not None:
            max_predict_samples = min(len(predict_dataset), data_args.max_predict_samples)
            predict_dataset = predict_dataset.select(range(max_predict_samples))

    # Use validation set as meta-dataset
    meta_dataset = None
    if training_args.hardness_aware_sampling:
        if not training_args.do_eval:
            raise ValueError("When hardness_aware_sampling=true, a validation set (do_eval=true) must be provided as meta-set.")
        meta_dataset = eval_dataset
        logger.info(f"Using validation set (size: {len(meta_dataset)}) as meta-set.")
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        # Compute and return three metrics
        ...
        # Output per-label accuracy using real_label_names
        for idx in range(len(real_label_names)):
            mask = labels == idx
            if mask.sum() == 0:
                continue
            name = real_label_names[idx]
            safe_name = name.replace(" ", "_").replace("/", "_")
            metrics[f"accuracy_{safe_name}"] = float((preds[mask] == idx).mean())
    ...
    use_meta = training_args.hardness_aware_sampling
    if use_meta:
        if training_args.embedding_dir is None:
            raise ValueError("`--embedding_dir` must be specified.")
        emb_path = os.path.join(training_args.embedding_dir, "train_embeddings.npy")
        if not os.path.isfile(emb_path):
            raise FileNotFoundError(f"Embedding file not found: {emb_path}")
        import torch
        emb_matrix = torch.from_numpy(np.load(emb_path)).float()

    TrainerClass = HardnessAwareTrainerForCls if use_meta else Trainer
    ...
    trainer = TrainerClass(
        ...
        # Pass extra arguments for meta-resampling
        **(
            {} if not use_meta else dict(
                meta_dataset=meta_dataset,
                train_dataset_len=len(train_dataset),
                hardness_aware_sampling=True,
                hardness_alpha=training_args.hardness_alpha,
                knn_k=training_args.knn_k,
                knn_lambda=training_args.knn_lambda,
                knn_build_freq=training_args.knn_build_freq,
    ...
    # Enable KNN neighbor-boost update at the end of each epoch.
    if use_meta and training_args.knn_lambda > 0 and training_args.weighted_sampling:
        trainer.add_callback(KnnEpochEndCallback(trainer))
    ...
    if training_args.do_predict:
        logger.info("*** Predict ***")
        # Keep labels to compute metrics for predictions as well.
        predictions_output = trainer.predict(predict_dataset, metric_key_prefix="predict")
        predictions = predictions_output.predictions
        metrics      = predictions_output.metrics

        # Post-process logits based on task type.
        if is_regression:
        ...
        # Custom print per-label accuracy and remove from metrics.
        per_label_acc = {
            k[len("predict_accuracy_"):]: v
            for k, v in metrics.items()
            if k.startswith("predict_accuracy_") and k != "predict_accuracy"
        }
        if per_label_acc:
            logger.info("***** predict per-label accuracy *****")
            for lbl, acc_val in per_label_acc.items():
                logger.info(f"{lbl:<45s}= {acc_val:8.4f}")
                metrics.pop(f"predict_accuracy_{lbl}", None)

        # Print and save prediction metrics.
